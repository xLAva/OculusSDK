...
/// Loads a list of users you can invite to your current room. These are pulled from your
/// friends list and filtered for relevance and interest. If your current room cannot be
/// joined this list will be empty.
///
...
/// First call ::ovr_Message_IsError() to check if an error occurred.
///
/// If no error ocurred, the message will contain a payload of type ::ovrRoomArrayHandle.
...
/// NOTE: Room datastores only allow string values. If you provide int or double values, this will return an error.
///
/// \param roomID the room you're the current owner of (check ovr_Room_GetOwner)
